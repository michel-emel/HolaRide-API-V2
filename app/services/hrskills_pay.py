...
def _detect_operator(phone: str) -> str:
    ...
    orange = {"655","656","657","658","659","687","688","689","690","691","692",
...
def initiate_cashout(phone: str, amount: float, trip_id: str) -> dict:
    ...
    phone_clean = phone.lstrip("+")
    operator    = _detect_operator(phone_clean)
    payload = {"operator": operator, "country": "CM", "phone_number": phone_clean, "amount": amount_int, "currency": "XAF"}
...
def get_payment_status(reference: str) -> dict:
    """Returns full payment data dict including status, error_code, failure_reason_category."""
    headers = {
        "Authorization": f"Bearer {settings.hrskills_key_a}",
        "X-Transaction-Token": _get_transaction_token(),
    }
